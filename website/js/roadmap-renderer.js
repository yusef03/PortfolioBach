...
function getText(entry, field, lang) {
    const key = `${field}_${lang}`;
    const fallback = `${field}_de`;
    return entry[key] || entry[fallback] || '';
}
function getChangelogText(entry, field, lang) {
    const key = `${field}_${lang}`;
    const fallback = `${field}_de`;
    return entry[key] || entry[fallback] || '';
}
...
function renderRoadmapTimeline(entries, lang) {
    if (entries.length === 0) {
...
function renderChangelogList(entries, lang, limit) {
...
function renderChangelogPage(entries, lang) {
    if (entries.length === 0) {
        const emptyText = lang === 'en' ? 'No releases yet.' : lang === 'ar' ? 'لا توجد إصدارات بعد.' : 'Noch keine Releases.';
...
const CL_MORE = { de: 'Details ansehen', en: 'View details', ar: 'عرض التفاصيل' };
const CL_CLOSE = { de: 'Schließen', en: 'Close', ar: 'إغلاق' };
let clDelegationReady = false;
let clLastTrigger = null;
function ensureChangelogModal(lang) {
    ...
    backdrop.querySelector('#cl-modal-close').addEventListener('click', closeChangelogModal);
    ...
    document.addEventListener('keydown', (e) => {
        ...
        if (e.key === 'Tab') {
            const f = backdrop.querySelectorAll('button, [href], [tabindex]:not([tabindex="-1"])');
            if (!f.length)
                return;
...
function openChangelogModal(card) {
    var _a, _b;
    const lang = getRoadmapLang();
    const backdrop = ensureChangelogModal(lang);
    clLastTrigger = card;
    const q = (sel) => { var _a; return ((_a = card.querySelector(sel)) === null || _a === void 0 ? void 0 : _a.textContent) || ''; };
    ...
    backdrop.querySelector('.cl-modal-body').innerHTML = ((_a = card.querySelector('.changelog-full-desc')) === null || _a === void 0 ? void 0 : _a.innerHTML) || '';
    backdrop.hidden = false;
    document.body.classList.add('cl-modal-open');
    requestAnimationFrame(() => backdrop.classList.add('is-open'));
    (_b = backdrop.querySelector('#cl-modal-close')) === null || _b === void 0 ? void 0 : _b.focus();
}
function closeChangelogModal() {
    ...
    clLastTrigger === null || clLastTrigger === void 0 ? void 0 : clLastTrigger.focus();
}
function setupChangelogDelegation(zone) {
    if (clDelegationReady)
        return;
    clDelegationReady = true;
    zone.addEventListener('click', (e) => {
        const card = e.target.closest('.changelog-card--clickable');
        if (card)
            openChangelogModal(card);
    });
...
function applyFilter(category) {
    ...
    // Chips aktualisieren
    document.querySelectorAll('.changelog-chip').forEach(chip => {
        chip.classList.toggle('is-active', chip.dataset.filter === category);
    });
    // Einträge ein-/ausblenden
    document.querySelectorAll('.changelog-timeline-entry').forEach(entry => {
...
// ─── Render: /roadmap.html ───────────────────────────────────────────────────
function renderGlobalRoadmapPage(isLangSwitch) {
    const lang = getRoadmapLang();
...
function renderChangelogFullPage(isLangSwitch) {
    ...
    const counts = { all: entries.length, feature: 0, fix: 0, refactor: 0, security: 0 };
    entries.forEach(e => { if (counts[e.category] !== undefined)
        counts[e.category]++; });
    document.querySelectorAll('.changelog-chip').forEach(chip => {
    ...
    if (metaEl)
        metaEl.textContent = String(entries.length);
}
// ─── Render: Projekt-Subseiten ───────────────────────────────────────────────
function renderProjectRoadmap(slug, isLangSwitch) {
    const lang = getRoadmapLang();
    ...
    const projectEntries = (typeof roadmapData !== 'undefined' ? roadmapData : [])
        .filter((e) => e.scope === 'project' && e.project_slug === slug);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    projectContainers.forEach((el) => {
        ...
        if (slug)
            renderProjectRoadmap(slug, false);
    });
});
// Sprachwechsel
window.addEventListener('languagechange', () => {
    ...
    projectContainers.forEach((el) => {
        const slug = el.id.replace('roadmap-project-', '');
        if (slug)
            renderProjectRoadmap(slug, true);
    });
});
