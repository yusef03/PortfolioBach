...
function getDescription(p, lang) {
    if (lang === 'en')
        return p.description_en || p.description_de;
    if (lang === 'ar')
        return p.description_ar || p.description_de;
    return p.description_de;
}
function getFeatureText(f, lang) {
    if (lang === 'en')
        return f.en || f.de;
    if (lang === 'ar')
        return f.ar || f.de;
...
function getStatusLabel(status, lang) {
    var _a;
    return ((_a = STATUS_LABELS[lang]) === null || _a === void 0 ? void 0 : _a[status]) || STATUS_LABELS['de'][status] || status;
}
function getStatusClass(status) {
    return `status-${status}`;
}
/** Baut den img/picture-Tag. WebP-Trick nur für lokale Pfade (nicht für Supabase-URLs). */
function renderImageTag(imageUrl, cssClass, alt) {
    if (!imageUrl) {
...
function activateRevealElements(container) {
...
function renderAll(isLanguageSwitch) {
    var _a;
    ...
    const heroItem = (_a = projectsData.find((p) => p.is_hero)) !== null && _a !== void 0 ? _a : projectsData[0];
    ...
    if (heroContainer && heroItem) {
        ...
        if (isLanguageSwitch)
            activateRevealElements(heroContainer);
    }
    // Archiv rendern
    if (archiveContainer) {
        ...
        if (isLanguageSwitch)
            activateRevealElements(archiveContainer);
    }
}
// --- Templates ---------------------------------------------------------------
function renderHeroTemplate(p, lang) {
...
function renderArchiveTemplate(p, lang) {
...
function renderGithubCardTemplate(lang) {
