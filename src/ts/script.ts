...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (navEntry?.type === "back_forward" && !window.location.hash) {
    ...
    if (document.readyState === "complete") window.setTimeout(restoreScroll, 60);
  }
  ...
  document.querySelectorAll<HTMLAnchorElement>(
    ".back-link, .thought-back-link, .thought-back-link-bottom"
  ).forEach((link) => {
    link.addEventListener("click", (e) => {
      ...
      } catch { /* kaputter Referrer — normaler Link greift */ }
    });
  });
...
/* ==========================================================================
  ...
  document.querySelectorAll<Element>(".reveal, .fade-in-up").forEach((el) => {
...
/* ==========================================================================
  ...
  if (langPill) {
    ...
    function updatePillUI(lang: string): void {
      langPill!.querySelectorAll<HTMLElement>(".lang-pill-seg").forEach(seg => {
        seg.classList.toggle("active", seg.dataset.lang === lang);
      });
    }

    // Core-Funktion: Tauscht DOM-Inhalte
    async function applyTranslations(lang: string): Promise<void> {
      ...
      document.querySelectorAll("[data-i18n]").forEach((el) => {
        ...
        if (text) {
          ...
          (el as HTMLElement)[text.includes("<") ? "innerHTML" : "textContent"] = text;
        }
      });
      ...
      document.querySelectorAll("[data-i18n-placeholder]").forEach((el) => {
        ...
        if (key && translationsObj[key]) {
          (el as HTMLInputElement).placeholder = translationsObj[key];
        }
      });
    ...
    function langUrl(lang: string): string {
      ...
      if (lang === "de") return path + hash;
      if (path === "/") return `/${lang}/${hash}`;
      return `/${lang}${path}${hash}`;
    }

    // Click: sanft ausblenden, dann zur Sprachvariante navigieren (echte URL, SEO-sauber).
    langPill.querySelectorAll<HTMLButtonElement>(".lang-pill-seg").forEach(seg => {
