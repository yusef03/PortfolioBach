...
// ─── Sprach-Labels (dynamische, vom Renderer erzeugte Strings) ────────────────
type Lang = 'de' | 'en' | 'ar'

const L: Record<Lang, {
  less: string; more: string
  repos: string; stars: string; followers: string; since: string
  languages: string; contributions: (n: string) => string
  tooltip: (n: number, date: string) => string
  empty: string
}> = {
  de: {
...
function ghLocale(lang: Lang): string {
...
// ─── Heatmap-HTML ─────────────────────────────────────────────────────────────
function renderContrib(data: GithubActivity, lang: Lang): string {
  const t = L[lang]
  ...
  const weekdays = [0, 1, 2, 3, 4, 5, 6].map(i =>
    `<span class="gh-weekday">${(i === 1 || i === 3 || i === 5) ? weekdayName(i) : ''}</span>`
  ).join('')

  // Wochen-Spalten
  const grid = cal.weeks.map((week, wi) =>
    `<div class="gh-week">${week.map(day => {
      if (!day.date) return `<span class="gh-day pad"></span>`
...
// ─── Stats-HTML ───────────────────────────────────────────────────────────────
function renderStats(data: GithubActivity, lang: Lang): string {
  const t = L[lang]
  const s = data.stats
  const tiles: string[] = []

  const tile = (icon: string, value: number, label: string, animate: boolean) =>
    `<div class="gh-stat">
       ${ICONS[icon]}
  ...
  if (s.followers > 0) tiles.push(tile('followers', s.followers, t.followers, true))
...
// ─── Sprachen-HTML ────────────────────────────────────────────────────────────
function renderLangs(data: GithubActivity, lang: Lang): string {
  if (!data.languages || data.languages.length === 0) return ''
  const t = L[lang]

  const colorOf = (name: string, i: number) => name === 'Other' ? LANG_OTHER : LANG_COLORS[i % LANG_COLORS.length]

  const segs = data.languages.map((l, i) =>
    `<span class="gh-lang-seg" style="width:${l.percent}%;background:${colorOf(l.name, i)}" title="${l.name} ${l.percent}%"></span>`
  ).join('')
...
function attachTooltips(root: HTMLElement, lang: Lang): void {
  const tip = ensureTooltip()
  root.querySelectorAll<HTMLElement>('.gh-day:not(.pad)[data-d]').forEach(cell => {
...
function ghActivateReveal(root: HTMLElement): void {
  const cells = root.querySelectorAll<HTMLElement>('.gh-day.gh-reveal')
  if (prefersReducedMotion()) { cells.forEach(c => c.classList.add('in')); return }
  requestAnimationFrame(() => cells.forEach(c => c.classList.add('in')))
}

function countUp(root: HTMLElement, lang: Lang): void {
  const targets = root.querySelectorAll<HTMLElement>('.gh-stat-value[data-count]')
  targets.forEach(el => {
    const target = Number(el.dataset.count || '0')
    if (prefersReducedMotion() || target === 0) { el.textContent = fmtNumber(target, lang); return }
    const duration = 1100
